...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) | 3 | 5 | 1 | 0 | 166.7% |
| 2 (Multiple Give) | 5 | 5 | 0 | 0 | 100% |
| 3 (Short Timeout) | 4 | 5 | 0 | 0 | 125% |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
- จะถูกให้ครั้งเดียวและครั้งถัดไปอาจ ล้มเหลว เพราะ Semaphore มีค่าได้แค่ 1

2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
- ต้องใช้ xSemaphoreGiveFromISR เพราะ Task ปกติไม่ปลอดภัยเมื่อเรียก xSemaphoreGive จาก ISR

3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
- แตกต่างจาก Queue ตรงที่มัน เก็บค่าได้เพียง 1 ใช้เพื่อ signaling ไม่ใช่ส่งข้อมูลหลายตัวเหมือน Queue
