...
| -------------------- | ---------- | ------ | --------- | ------------ | ---------------------------------------------------------------------------------------- |
| 1 (With Mutex)       | 13         | 0      | 0         | 100%         | ทุก task ใช้ mutex จัดการ shared resource ไม่มี race condition                           |
| 2 (No Mutex)         | 3–4        | 4–5    | 2–3       | ~40–50%      | เกิด race condition, ข้อมูล buffer และ counter มีความผิดพลาด (corrupted)                 |
| 3 (Changed Priority) | 13         | 0      | 0         | 100%         | ใช้ priority inheritance ช่วยลด priority inversion, resource ถูกเข้าถึงตามลำดับ priority |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
- การเข้าถึง shared resource อาจทำให้เกิด data corruption

2. Priority Inheritance ทำงานอย่างไร?
- ยกระดับ priority ของ task ต่ำที่ถือ mutex เมื่อ task สูงรอ → ลด priority inversion

3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
- กำหนดลำดับการเข้าถึง shared resource → task ที่ priority สูงกว่าจะได้ก่อน
