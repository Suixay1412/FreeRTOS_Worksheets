...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) | ใช้งาน | ปกติ | ปกติ | ปกติ | ทำงานครบ | ระบบทำงานปกติทุก Task ไม่มี delay |
| 2 (ไม่มี Sensor) | ขาด | ปกติ | ปกติ | ปกติ | ลดลง | Task Sensor หยุด ทำให้ Total งานลดลง, แต่ Task อื่นทำงานต่อได้ |
| 3 (Network เร็ว) | ใช้งาน | ปกติ | เร็ว | ปกติ | เพิ่ม | Network ส่งข้อมูลเร็วขึ้น ทำให้ Total งานเพิ่มและ Task อื่นรอข้อมูลน้อยลง |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
- รู้ได้จาก Queue Set handle ซึ่งบอกว่า event หรือข้อมูลมาจาก Queue ไหน

2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
- Task จะได้รับ Queue ที่พร้อมก่อน ตามลำดับที่ xQueueSelectFromSet() คืนค่า

3. Queue Sets ช่วยประหยัด CPU อย่างไร?
- ช่วยให้ Task รอหลาย Queue/ Semaphore พร้อมกันโดยไม่ต้อง polling ทุก Queue ลดการใช้ CPU
